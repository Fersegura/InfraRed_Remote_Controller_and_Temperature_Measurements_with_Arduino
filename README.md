...
When the MQTT protocol was implemented, we experimented several issues (the microcontroller crashed every now and then). We figured out that was due to the buttons connected to the D3, D4 and D8 pins. The logic was only correct for the D8 pin, because it has a pull down internal resistor. To solve the problem we intverted the physical logic of the buttons connected to D3 and D4 (they are now buttons with pull up resistors). 
